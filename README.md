...
Как в **§3 ТЗ**, типичная страница вуза часто:

- даёт информацию «сухо», без понятных следующих шагов;
- не персонализирует списки программ под интересы и баллы;
- не собирает документы и дедлайны в единый **action plan**;
- не создаёт ощущение активного помощника.

Прототип закрывает это за счёт связной логики профиля, fit score, чек-листа и таймлайна.
...
- Backend профиля и прокси ИИ · загрузка файлов в IndexedDB · несколько вузов в мок-каталоге · **Qwen** для чата, обзоров и разбора достижений · локализация **kk / ru / en** · каталог с поиском и сравнением вузов · избранное программ и вузов.
...
- **Университеты и программы** — мок в **`src/mockData.ts`** и связанных модулях каталога; это **не** официальные прайсы или правила приёма, а демонстрационный контент для хакатона.
...
Упрощения относительно **§10 ТЗ** (блоки A–K):

- **Navbar:** нет дословного меню «Universities & Ranking · About · Apply · Blog» платформы QApp; вместо этого навигация под сценарий прототипа (главная, дашборд, профиль, якоря к секциям, переключатель языка). Нет аватара и отдельной кнопки «Apply to [University]» — есть CTA в контенте и ссылки на официальный сайт в карточках.
- **Hero:** нет обязательного двуязычного названия вуза в одной строке (можно усилить контентом в данных).
- **AI Fit:** статусы Qualified / Strong Match / Partial Match могут отображаться иначе (процент + текст); gaps и next steps частично через списки и executive summary Qwen.
- **Programs:** фильтр по «Fit Score» как отдельный контроль не выделен — сортировка и скор на карточках задаются логикой грида.
- **Footer по §10.K** — отдельный футер с Legal / My Applications как на платформе QApp **не свёрстан** (можно добавить как thin footer).
...
- скриншоты и деплой-ссылка в README для жюри.
